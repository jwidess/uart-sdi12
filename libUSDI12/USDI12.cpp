...
USDI12::USDI12(volatile uint8_t* enTxPort, uint8_t enTxBit,
  ...
  // YES, this case tree is ugly and bad, but it works for now!
  switch (uartNum) {
    case 0:
      _ucsrNa = &UCSR0A;
      _ucsrNb = &UCSR0B;
...
void USDI12::setBit(volatile uint8_t* port, uint8_t bit) { *port |= bit; }

void USDI12::clearBit(volatile uint8_t* port, uint8_t bit) { *port &= ~bit; }
